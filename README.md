...
- **多显示器 DPI 自适应**:布局和字体都按当前显示器缩放(100% / 150% / 200% …),
  在两块不同缩放的屏幕间拖动时自动重建字体并重新计算卡片尺寸。
- **中英分字体**:中文用 `HYYouYuan 55W`(汉仪有圆),英文与数字用 `JetBrains Mono`。
  两者都是等宽,中文 `汉字宽度 = 2 × 英文宽度`,列对齐因此是精确的。
...
| 滚轮 | 上下滚动 |
| 右键 | 刷新 / 暂停 / 打开请求页 / 重新登录 / 清除凭据 / 总在最前 / 退出 |
...
```jsonc
{
  ...
  "alwaysOnTop": true,
  "window": { "x": -1, "y": -1, "width": 452, "height": 916 }
}
```
